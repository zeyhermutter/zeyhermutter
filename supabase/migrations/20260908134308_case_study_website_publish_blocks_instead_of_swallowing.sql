-- Nachbesserung zur vorigen Migration.
--
-- Dort stand: "if new.status <> 'PUBLISHABLE' then new.website_published := false".
-- Das lief vor der Constraint-Pruefung und hat damit jeden Versuch, eine noch
-- nicht freigegebene Fallstudie zu veroeffentlichen, stillschweigend
-- zurueckgesetzt. Kein Fehler, keine Meldung -- der Haken im Formular waere
-- beim naechsten Laden einfach wieder weg gewesen, ohne dass jemand erfaehrt,
-- warum. Das System soll warnen und blockieren, nicht schlucken.
--
-- Jetzt sind es zwei getrennte Faelle:
--   Status verlaesst PUBLISHABLE  -> Seite raeumen, ohne Nachfrage (Widerruf)
--   ausdruecklich veroeffentlichen ohne PUBLISHABLE -> Fehler mit Begruendung
