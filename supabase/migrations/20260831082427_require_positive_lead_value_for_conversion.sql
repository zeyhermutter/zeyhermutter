...
begin
  ...
  if nullif(trim(coalesce(v_lead.property_street,'')),'') is null
     ...
     or nullif(trim(coalesce(v_lead.property_city,'')),'') is null then raise exception 'LEAD_ADDRESS_REQUIRED' using errcode='22023'; end if;
  ...
  if coalesce(v_lead.price_expectation,0) <= 0 and coalesce(v_lead.estimated_market_value,0) <= 0 then raise exception 'LEAD_PRICE_OR_MARKET_VALUE_REQUIRED' using errcode='22023'; end if;
  ...
  insert into public.properties(internal_title,property_type,transaction_type,purchase_price,living_area_sqm,plot_area_sqm,rooms,year_built,condition,tenancy_status,internal_notes,primary_responsible_user,created_by,updated_by)
  values(v_title,v_lead.property_type,'SALE',v_lead.price_expectation,v_lead.living_area_sqm,v_lead.plot_area_sqm,v_lead.rooms,v_lead.year_built,v_lead.property_condition,v_lead.occupancy_status,concat_ws(E'\n',nullif(v_lead.internal_notes,''),case when v_lead.estimated_market_value is not null then 'Bewerteter Marktwert aus Lead: ' || v_lead.estimated_market_value::text || ' EUR' end,case when v_lead.message is not null then 'Lead-Hintergrund: ' || v_lead.message end,'Verkaufshorizont aus Lead: ' || v_lead.desired_sale_horizon,'Konvertiert aus ' || v_lead.lead_number),v_lead.primary_responsible_user,v_user,v_user)
  returning id into v_property_id;

  insert into public.property_addresses(property_id,street,house_number,postal_code,city,district,country,public_address_mode,created_by,updated_by)
  values(v_property_id,trim(v_lead.property_street),trim(v_lead.property_house_number),trim(v_lead.property_postal_code),trim(v_lead.property_city),nullif(trim(coalesce(v_lead.property_district,'')),''),v_lead.property_country,'CITY_ONLY',v_user,v_user);

  insert into public.property_owners(property_id,contact_id,ownership_percentage,ownership_type,primary_contact,valid_from,created_by,updated_by)
  values(v_property_id,v_lead.contact_id,100.00,'SOLE_OWNER',true,current_date,v_user,v_user);

  perform set_config('app.lead_conversion','1',true);
  update public.leads set converted_property_id=v_property_id,converted_at=now(),converted_by=v_user where id=v_lead.id and version=p_expected_version;
