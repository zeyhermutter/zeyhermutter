...
const STATUS_HELP:Record<string,string>={NEW:"Neu eingegangen",CONTACTED:"Erstkontakt erfolgt",QUALIFIED:"Auftrag grundsätzlich passend",APPOINTMENT:"Termin vereinbart",VALUATION:"Bewertung läuft",OFFER:"Maklerangebot liegt vor",WON:"Auftrag gewonnen",NURTURE:"Später wieder aufnehmen",LOST:"Auftrag verloren"};
...
function errorMessage(message:string){
  if(message.includes("lead source required"))return "Ab Qualifiziert muss zuerst eine Leadquelle gespeichert werden.";
  if(message.includes("invalid lead status transition"))return "Dieser Statuswechsel ist im Workflow nicht erlaubt.";
  if(message.includes("lost reason required"))return "Für Verloren ist ein Verlustgrund erforderlich.";
  if(message.includes("converted lead business data is immutable"))return "Dieser Lead wurde bereits in eine Immobilie übernommen und ist fachlich gesperrt.";
...
export async function loader({request,context}:Route.LoaderArgs){
  ...
  return data({rows:filtered,sources:sources??[],profiles:profiles??[],profileMap,pipeline,transitionMap,profile,filters:{q,status,source,responsible,followup,archived,page},total,totalPages},{headers:responseHeaders()});
}
...
export default function Leads(){
  const {rows,sources,profiles,profileMap,pipeline,transitionMap,profile,filters,total,totalPages}=useLoaderData<typeof loader>();
  ...
  return <main className="editor-shell lead-shell">
    ...
    <div className="lead-page-width">
      ...
      <section className="data-card lead-workflow-board"><div className="card-head"><div><p className="eyebrow">Status & Workflow</p><h2>Verkäufer-Pipeline</h2></div><Link className="subtle-link" to={filterHref("ALL")}>Alle Leads</Link></div><p className="lead-pipeline-note">Die Stufen zeigen den aktuellen Bestand. Klick auf eine Stufe filtert die Liste; den nächsten zulässigen Schritt kannst du direkt beim Lead ausführen.</p><div className="lead-workflow-track">{pipeline.map((stage,index)=><Link key={stage.status} className={`lead-workflow-step${filters.status===stage.status?" active":""}${stage.status==="LOST"?" outcome-lost":""}${stage.status==="WON"?" outcome-won":""}`} to={filterHref(stage.status)}><span className="lead-workflow-index">{index+1}</span><span className="lead-workflow-copy"><strong>{STATUS_LABELS[stage.status]}</strong><small>{STATUS_HELP[stage.status]}</small></span><span className="lead-workflow-count">{stage.count}</span>{stage.overdue>0?<span className="lead-workflow-overdue">{stage.overdue} überfällig</span>:null}</Link>)}</div></section>
