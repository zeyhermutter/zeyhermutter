...
export default function Leads(){
  ...
  const renderStage=(status:string,index?:number)=>{const stage=pipelineByStatus[status]??{status,count:0,overdue:0};return <Link key={status} className={`lead-workflow-step${filters.status===status?" active":""}${status==="LOST"?" outcome-lost":""}${status==="WON"?" outcome-won":""}`} to={filterHref(status)}>{index!==undefined?<span className="lead-workflow-index">{index+1}</span>:<span className="lead-workflow-index outcome">↳</span>}<span className="lead-workflow-copy"><strong>{STATUS_LABELS[status]}</strong><small>{STATUS_HELP[status]}</small></span><span className="lead-workflow-count">{stage.count}</span>{stage.overdue>0?<span className="lead-workflow-overdue">{stage.overdue} überfällig</span>:null}</Link>;};
  return <main className="editor-shell lead-shell">
    ...
    <div className="lead-page-width">
      ...
      <section className="data-card lead-workflow-board"><div className="card-head"><div><p className="eyebrow">Status & Workflow</p><h2>Verkäufer-Pipeline</h2></div><Link className="subtle-link" to={filterHref("ALL")}>Alle Leads</Link></div><p className="lead-pipeline-note">Die Hauptstrecke führt von Neu bis Gewonnen. Nurture und Verloren sind Nebenpfade. Klick auf eine Stufe filtert die Liste; zulässige Statuswechsel kannst du direkt beim Lead ausführen.</p><div className="lead-workflow-main">{MAIN_STATUSES.map((status,index)=>renderStage(status,index))}</div><div className="lead-workflow-outcomes"><span className="lead-workflow-outcome-label">Nebenpfade</span>{OUTCOME_STATUSES.map(status=>renderStage(status))}</div></section>
