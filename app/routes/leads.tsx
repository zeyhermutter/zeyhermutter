...
export default function Leads(){
  ...
  return <main className="editor-shell lead-shell">
    ...
    <div className="lead-page-width">
      ...
      <section className="data-card lead-workflow-board"><div className="card-head"><div><p className="eyebrow">Status & Workflow</p><h2>Verkäufer-Pipeline</h2></div><Link className="subtle-link" to={filterHref("ALL")}>Alle Leads</Link></div><p className="lead-pipeline-note">Die Hauptstrecke führt von Neu bis Gewonnen. Später nachfassen und Verloren sind Nebenpfade. Klick auf eine Stufe filtert die Liste; zulässige Statuswechsel kannst du direkt beim Lead ausführen.</p><div className="lead-workflow-main">{MAIN_STATUSES.map((status,index)=>renderStage(status,index))}</div><div className="lead-workflow-outcomes"><span className="lead-workflow-outcome-label">Nebenpfade</span>{OUTCOME_STATUSES.map(status=>renderStage(status))}</div></section>
