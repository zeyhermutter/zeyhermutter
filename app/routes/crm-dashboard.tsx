...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  if (contactError || taskError || organizationError) {
...
export default function CrmDashboard() {
  const { profile, contacts, tasks, organizationCount } = useLoaderData<typeof loader>();
    ...
    <main className="app-shell">
      ...
      <section className="app-content">
        <header className="app-header">
          ...
          <div className="header-actions"><Link className="primary-button link-button" to="/crm/contacts/new">+ Kontakt</Link><span className="badge">STAGING</span></div>
        </header>
        ...
        <div className="metric-grid">
          ...
          <article className="metric-card"><span>Offene Aufgaben</span><strong>{tasks.length}</strong><small>nächste Wiedervorlagen</small></article>
        </div>

        <div className="dashboard-grid">
          <section className="data-card">
            <div className="card-head"><div><p className="eyebrow">Kontakte</p><h2>CRM</h2></div><Link className="subtle-link" to="/crm/contacts/new">Neu anlegen</Link></div>
              ...
              <div className="data-list">
                {contacts.map((contact) => (
                  <div className="data-row" key={contact.id}>
                    ...
                    <div className="row-meta"><Link className="subtle-link" to={`/crm/contacts/${contact.id}/relations`}>Arbeitsbereich</Link><Link className="subtle-link" to={`/crm/contacts/${contact.id}`}>Stammdaten</Link></div>
                  </div>
                ))}
              </div>
            )}
          ...
          <section className="data-card">
            <div className="card-head"><div><p className="eyebrow">Wiedervorlagen</p><h2>Aufgaben</h2></div></div>
