...
export default function CrmSearch() {
  ...
  return <main className="editor-shell"><header className="editor-header"><div><Link className="back-link" to="/crm">← CRM</Link><p className="eyebrow">ZeyherMutterOS</p><h1 className="editor-title">Globale Suche</h1></div><div className="header-user"><span className="badge">STAGING</span><small>{profile.display_name}</small></div></header><section className="data-card"><Form method="get" className="search-bar"><input name="q" defaultValue={q} placeholder="Kontakt, Lead, Immobilie, Suchprofil, Anfrage, Nummer oder Adresse …" minLength={2} autoFocus/><label className="checkbox-row"><input type="checkbox" name="archived" value="1" defaultChecked={includeArchived}/><span>Archiv einbeziehen</span></label><button className="primary-button" type="submit">Suchen</button></Form></section><section className="data-card"><div className="card-head"><div><p className="eyebrow">Ergebnisse</p><h2>{q.length>=2?`${results.length} Treffer`:"Suchbegriff eingeben"}</h2></div><div className="inline-actions"><Link className="subtle-link" to="/search-profiles">Suchprofile</Link><Link className="subtle-link" to="/inquiries">Anfragen</Link><Link className="subtle-link" to="/leads">Leads</Link><Link className="subtle-link" to="/properties">Immobilien</Link></div></div>{q.length>=2&&results.length===0?<p className="empty-state">Keine Treffer gefunden.</p>:null}<div className="data-list">{results.map(result=><Link className="data-row data-row-link" to={target(result)} key={`${result.entity_type}-${result.entity_id}`}><div><strong>{result.title}</strong><small>{LABELS[result.entity_type]??result.entity_type} · {result.reference} · {result.subtitle}</small></div><div className="row-meta"><span>{result.archived?"ARCHIVIERT":result.status}</span><small>{new Intl.DateTimeFormat("de-DE",{dateStyle:"short",timeZone:"Europe/Berlin"}).format(new Date(result.updated_at))}</small></div></Link>)}</div></section></main>;
}
