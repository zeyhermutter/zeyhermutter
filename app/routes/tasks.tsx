...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  let query = supabase.from("tasks").select("id,task_number,title,description,status,priority,due_at,contact_id,property_id,lead_id,inquiry_id,search_profile_id,viewing_id,responsible_user,version").is("archived_at", null).order("due_at", { ascending: true }).limit(200);
  if (status === "OPEN") query = query.in("status", ["OPEN", "IN_PROGRESS"]);
  else if (status !== "ALL") query = query.eq("status", status);

  const [{ data: tasks, error: taskError }, { data: contacts, error: contactError }, { data: profiles, error: profileError }, { data: properties, error: propertyError }, { data: leads, error: leadError }, { data: inquiries, error: inquiryError }, { data: searchProfiles, error: searchProfileError }, { data: viewings, error: viewingError }] = await Promise.all([
    query,
  ...
  return data({ tasks: tasks ?? [], contacts: contacts ?? [], profiles: profiles ?? [], properties: properties ?? [], leads: leads ?? [], inquiries: inquiries ?? [], searchProfiles: searchProfiles ?? [], viewings: viewings ?? [], contactMap: map(contacts ?? [], "id"), profileMap: Object.fromEntries((profiles ?? []).map((x: any) => [x.user_id, x.display_name])), propertyMap: map(properties ?? [], "id"), leadMap: map(leads ?? [], "id"), inquiryMap: map(inquiries ?? [], "id"), searchProfileMap: map(searchProfiles ?? [], "id"), viewingMap: map(viewings ?? [], "id"), profile, status }, { headers: responseHeaders() });
}
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (intent === "task_status") {
    ...
    if (!updated) return data<ActionResult>({ error: "Die Aufgabe wurde zwischenzeitlich geändert. Bitte neu laden." }, { status: 409, headers: responseHeaders() });
    return redirect(`/crm/tasks?status=${encodeURIComponent(new URL(request.url).searchParams.get("status") ?? "OPEN")}`, { headers: responseHeaders() });
  }
...
export default function Tasks() {
  const { tasks, contacts, profiles, properties, leads, inquiries, searchProfiles, viewings, contactMap, profileMap, propertyMap, leadMap, inquiryMap, searchProfileMap, viewingMap, profile, status } = useLoaderData<typeof loader>();
  ...
  return (
    <main className="editor-shell">
      <header className="editor-header">
        <div><Link className="back-link" to="/crm">← CRM</Link><p className="eyebrow">CRM</p><h1 className="editor-title">Aufgaben & Wiedervorlagen</h1></div>
      ...
      <div className="tasks-page tasks-page-modal-layout module04-content">
        <TaskCreateModal
          ...
          searchProfiles={searchProfileOptions}
          inquiries={inquiryOptions}
          viewings={viewingOptions}
        />
        <section className="data-card">
          <div className="card-head">
            <div><p className="eyebrow">Wiedervorlagen</p><h2>Aufgaben</h2></div>
            <Form method="get"><select name="status" defaultValue={status} onChange={(e) => e.currentTarget.form?.submit()}><option value="OPEN">Offen</option><option value="ALL">Alle</option><option value="DONE">Erledigt</option><option value="CANCELLED">Abgebrochen</option></select></Form>
          </div>
          <div className="data-list">
            {tasks.map((task: any) => {
              ...
              const lead = task.lead_id ? leadMap[task.lead_id] : null;
              const inquiry = task.inquiry_id ? inquiryMap[task.inquiry_id] : null;
              const sp = task.search_profile_id ? searchProfileMap[task.search_profile_id] : null;
              const viewing = task.viewing_id ? viewingMap[task.viewing_id] : null;
              ...
              return (
                <div className="data-row task-list-row" key={task.id}>
                  <div>
                    <div className="task-list-title"><strong>{task.title}</strong><span className={`task-status-badge status-${String(task.status).toLowerCase()}`}>{STATUS[task.status] ?? task.status}</span></div>
                    <small>{task.task_number} · {PRIORITY[task.priority] ?? task.priority} · {overdue ? "ÜBERFÄLLIG · " : ""}fällig {formatDate(task.due_at)} · verantwortlich: {task.responsible_user ? (profileMap[task.responsible_user] ?? "Benutzer") : "—"}</small>
                    <small className="task-context-label">{contextLabel}</small>
                  </div>
                  <div className="task-list-actions">
                    <TaskModal task={task} responsibleName={task.responsible_user ? (profileMap[task.responsible_user] ?? "Benutzer") : "—"} contextLabel={contextLabel} contextHref={contextHref} />
                    <Form method="post" className="inline-actions">
                      <input type="hidden" name="task_id" value={task.id} />
                      <input type="hidden" name="version" value={task.version} />
                      <select name="responsible_user" defaultValue={task.responsible_user ?? ""}>{profiles.map((x: any) => <option key={x.user_id} value={x.user_id}>{x.display_name}</option>)}</select>
