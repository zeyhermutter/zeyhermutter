...
export default function PropertyPublication(){
 ...
 return <main className="editor-shell"><header className="editor-header"><div><Link className="back-link" to={`/properties/${property.id}`}>← Objektakte</Link><p className="eyebrow">Modul 05 · Website & Exposés</p><h1 className="editor-title">Veröffentlichung</h1><p className="editor-meta">{property.property_number} · {property.internal_title}</p></div><div className="header-user"><span className="publication-state">{stateLabel(publication,liveVersion)}</span><span className="badge">STAGING</span><small>{profile.display_name}</small></div></header>
 <div className="publication-page">{result?.error?<div className="form-error">{result.error}</div>:null}
  ...
  {publication?<section className="data-card" id="freigabe"><div className="card-head"><div><p className="eyebrow">Publikations-Checkliste</p><h2>Vor Veröffentlichung prüfen</h2></div><span>{blockers.length?`${blockers.length} offen`:"Bereit"}</span></div>
   ...
   {blockers.length?<div className="publication-blockers"><strong>Vor Veröffentlichung noch erforderlich</strong>{blockers.map((item:any)=><a key={item.id} href={`#${item.id}`}>→ {item.label}</a>)}</div>:<div className="publication-ready">Alle blockierenden Prüfungen sind erfüllt.</div>}
  </section>:null}
