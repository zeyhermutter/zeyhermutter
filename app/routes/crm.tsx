...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  if (!profile || profile.status !== "ACTIVE") {
    return data(
      { accessPending: true, profile, contacts: [], tasks: [] },
  ...
  return data(
    { accessPending: false, profile, contacts: contacts ?? [], tasks: tasks ?? [] },
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p className="auth-copy">Das Benutzerkonto ist vorhanden, benötigt aber noch eine aktive Rolle.</p>
          <Form method="post" action="/logout"><button className="secondary-button" type="submit">Abmelden</button></Form>
        </section>
      </main>
    );
  }
      ...
      <section className="app-content">
        ...
        <div className="dashboard-grid">
          <section className="data-card">
            <div className="card-head"><div><p className="eyebrow">CRM</p><h2>Kontakte</h2></div><Link className="subtle-link" to="/crm/contacts/new">Neu anlegen</Link></div>
            {contacts.length === 0 ? <p className="empty-state">Noch keine Kontakte vorhanden.</p> : (
              <div className="data-list">
                {contacts.map((contact) => (
                  <Link className="data-row data-row-link" to={`/crm/contacts/${contact.id}`} key={contact.id}>
