...
function scenarioEffect(
  scenario: SalesReadinessScenarioViewModel,
  base?: SalesReadinessScenarioViewModel,
) {
        ...
        <section className="data-card readiness-empty-start">
          ...
          {canWrite ? (
            <Form method="post">
              ...
              <button className="primary-button" type="submit">Verkaufsfertig-Check starten</button>
            </Form>
  ...
  return (
    <div className="readiness-workspace">
      <section className="readiness-mode-banner preview" role="status">
        <strong>BETA · Datenbank aktiv</strong>
        <span>Änderungen werden versioniert gespeichert. Finalisierte Revisionen sind unveränderlich.</span>
      </section>
      ...
      <section className="readiness-overview data-card">
        ...
        <div className="readiness-actions">
          {check.status === "DRAFT" && canWrite ? (
            <Form method="post">
              <input type="hidden" name="_intent" value="ready" />
              <input type="hidden" name="check_id" value={checkId} />
              <input type="hidden" name="version" value={check.version} />
              <button className="secondary-button" type="submit">Zur Prüfung markieren</button>
            </Form>
          ) : null}
          {check.status === "READY_FOR_REVIEW" && canFinalize ? (
            <Form method="post">
              ...
              <input type="hidden" name="check_id" value={checkId} />
              <input type="hidden" name="version" value={check.version} />
              <button className="primary-button" type="submit">Finalisieren</button>
            </Form>
          ) : null}
          {check.status === "FINALIZED" && canFinalize ? (
            <Form method="post">
              <input type="hidden" name="_intent" value="revision" />
              <input type="hidden" name="check_id" value={checkId} />
              <input type="hidden" name="version" value={check.version} />
              <button className="secondary-button" type="submit">Revision anlegen</button>
            </Form>
          ) : null}
        </div>
      </section>
      ...
      <Form method="post" className="data-card readiness-edit-card">
        <input type="hidden" name="_intent" value="save_check" />
        <input type="hidden" name="check_id" value={checkId} />
        <input type="hidden" name="version" value={check.version} />
      ...
      <section className="readiness-section">
        <div className="readiness-section-head">
          ...
          <p>Mehrerlös und wirtschaftlicher Vorteil sind Schätzwerte aus den eingetragenen Korridoren – keine Verkaufspreisgarantie.</p>
        </div>
          ...
          {viewModel.scenarios.map((scenario) => {
            ...
            return (
              <Form method="post" className={`readiness-scenario ${scenario.recommended ? "recommended" : ""}`} key={scenario.id}>
                <input type="hidden" name="_intent" value="save_scenario" />
                <input type="hidden" name="check_id" value={checkId} />
                <input type="hidden" name="version" value={check.version} />
                ...
                <label className="readiness-recommend"><input type="checkbox" name="is_recommended" defaultChecked={scenario.recommended} disabled={!editable} /> Dieses Szenario empfehlen</label>
                {effect ? (
                  <div className="readiness-calculation">
                    <strong>Schätzwerte gegenüber Ist-Zustand</strong>
                    <span>Geschätzter Mehrerlös: {euro(effect.upliftMin)} bis {euro(effect.upliftMax)}</span>
                    <span>Potenzieller wirtschaftlicher Vorteil: {effect.advantageMin === null || effect.advantageMax === null ? "Investition ergänzen" : `${euro(effect.advantageMin)} bis ${euro(effect.advantageMax)}`}</span>
                  </div>
                ) : null}
        ...
        <div className="readiness-measure-list">
          {viewModel.measures.map((measure) => (
            <Form method="post" className="readiness-measure-editor" key={measure.id}>
              <input type="hidden" name="_intent" value="save_measure" />
              <input type="hidden" name="check_id" value={checkId} />
              <input type="hidden" name="version" value={check.version} />
              ...
              <div className="readiness-form-grid compact">
                ...
                <label><span>Kosten min.</span><input name="cost_min" type="number" min="0" step="50" defaultValue={valueOrBlank(measure.costMin)} disabled={!editable} /></label>
                <label><span>Kosten max.</span><input name="cost_max" type="number" min="0" step="50" defaultValue={valueOrBlank(measure.costMax)} disabled={!editable} /></label>
        ...
        {editable ? (
          <Form method="post" className="readiness-measure-editor new-measure">
            ...
            <div className="readiness-form-grid compact">
              ...
              <label><span>Status</span><select name="status" defaultValue="OPEN">{Object.entries(MEASURE_STATUS_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
              <label><span>Sortierung</span><input name="sort_order" type="number" min="0" step="10" defaultValue="100" /></label>
            </div>
            <button className="primary-button" type="submit">Maßnahme hinzufügen</button>
          </Form>
        ) : null}

        {check.status === "FINALIZED" && canTask ? (
          <Form method="post" className="readiness-task-create">
            <input type="hidden" name="_intent" value="create_tasks" />
            <input type="hidden" name="check_id" value={checkId} />
            <input type="hidden" name="version" value={check.version} />
            <h3>CRM-Aufgaben aus Maßnahmen erzeugen</h3>
            <p>Nur dringend empfohlene, empfohlene oder optionale Maßnahmen können übernommen werden; bestehende Aufgaben werden nicht dupliziert.</p>
            <div className="readiness-task-options">
              {viewModel.measures
                .filter((measure) => ["URGENTLY_RECOMMENDED", "RECOMMENDED", "OPTIONAL"].includes(measure.decision) && measure.status !== "DISMISSED")
                .map((measure) => (
            ...
            <button className="primary-button" type="submit">Ausgewählte CRM-Aufgaben erzeugen</button>
          </Form>
        ) : null}
      </section>

      <section className="readiness-disclaimer">
        <strong>Foto-Dokumentation vorbereitet:</strong> Das Datenmodell kann Vorher-, Währenddessen- und Nachher-Medien mit separater Marketingfreigabe referenzieren. Der tatsächliche Foto-Storage wird in diesem Durchlauf bewusst nicht automatisch aktiviert.
      </section>
    </div>
  );
}
