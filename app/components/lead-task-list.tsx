    ...
    <>
      <div className="lead-task-list">
        {tasks.map((task) => (
          ...
          >
            <div><strong>{task.title}</strong><small>{task.task_number} · {beschrifte(AUFGABENSTATUS, task.status)} · {formatDate(task.due_at)}</small></div>
            <span>Details →</span>
          </button>
        ))}
        ...
        <div className="lead-modal-backdrop" role="presentation" onMouseDown={() => setSelected(null)}>
          <section className="lead-modal" role="dialog" aria-modal="true" aria-labelledby="lead-task-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="card-head">
              <div><p className="eyebrow">{selected.task_number} · {beschrifte(AUFGABENPRIORITAET, selected.priority)}</p><h2 id="lead-task-title">{selected.title}</h2></div>
              <button className="text-button" type="button" onClick={() => setSelected(null)}>Schließen ×</button>
            </div>
            <div className="lead-task-modal-meta">
              <span>Status: <strong>{beschrifte(AUFGABENSTATUS, selected.status)}</strong></span>
